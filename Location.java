...
public class Location implements Comparable<Location> {
    ...
    public double distanceTo(Location other) {
        double a = Math.toRadians(this.lat);
        double b = Math.toRadians(this.lon);
        double c = Math.toRadians(other.lat);
        double d = Math.toRadians(other.lon);

        return 6359.83 * (Math.acos(Math.sin(a) * Math.sin(c) +
